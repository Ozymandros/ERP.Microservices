...
var serviceName = '${namePrefix}-crm-service'
var imageName = '${basePrefix}-crm-service-${envSlug}'

module crmService 'container-app-service.bicep' = {
  name: serviceName
  params: {
    name: serviceName
    location: location
    ...
    daprAppPort: 8080
    minReplicas: finopsMinReplicas  // OPTIMITZACIO FINOPS: Escala a zero - no paga quan no s'usa
    maxReplicas: finopsMaxReplicas  // Minim possible
    cpu: finopsCpuCores  // Minim absolut (250m cores)
    memory: finopsMemory  // Minim absolut (512 MiB)
    ...
    azdServiceName: 'crm-service'  // Must match service name in azure.yaml
  }
}

output id string = crmService.outputs.id
output name string = crmService.outputs.name
output uri string = crmService.outputs.uri
output fqdn string = crmService.outputs.fqdn
@description('Managed Identity Principal ID for RBAC role assignments (Phase 3)')
output managedIdentityPrincipalId string = crmService.outputs.managedIdentityPrincipalId
