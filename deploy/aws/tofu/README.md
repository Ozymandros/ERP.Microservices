...
**Default variable values target a Free Tier–constrained lab** (1× On-Demand `t3.small`, no NAT, no backup bucket). Scale instance type when billing allows.
...
| `node_instance_types` | `["t3.small"]` (Cheap Tier) | `["t3.xlarge"]` / `["m6i.large"]` for ERP+SQL |
